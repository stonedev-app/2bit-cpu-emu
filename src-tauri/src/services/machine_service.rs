...
mod tests {
    ...
    fn test_decode_instruction() {
        assert_eq!(decode_instruction(0b00_00), (0b00, 0b00));
        assert_eq!(decode_instruction(0b00_11), (0b00, 0b11));
        assert_eq!(decode_instruction(0b01_10), (0b01, 0b10));
        assert_eq!(decode_instruction(0b11_01), (0b11, 0b01));
    }
    ...
    fn test_fetch_instruction() {
        ...
        machine.rom[0] = 0b00_01;
        machine.rom[1] = 0b01_10;
        machine.rom[2] = 0b10_11;
        machine.rom[3] = 0b11_00;

        machine.cpu.pc_counter = 0;
        assert_eq!(fetch_instruction(&machine), 0b00_01);

        machine.cpu.pc_counter = 1;
        assert_eq!(fetch_instruction(&machine), 0b01_10);

        machine.cpu.pc_counter = 2;
        assert_eq!(fetch_instruction(&machine), 0b10_11);

        machine.cpu.pc_counter = 3;
        assert_eq!(fetch_instruction(&machine), 0b11_00);
    }
    ...
    fn test_execute_add() {
        ...
        let result = execute_add(&cpu, 0b10);
        assert_eq!(result.a_reg, 0b11); // 1 + 2 = 3
        assert_eq!(result.c_flag, false); // キャリーなし
        assert_eq!(result.pc_counter, 1); // PC + 1
    }
    ...
    fn test_execute_add_with_carry() {
        ...
        let result = execute_add(&cpu, 0b01);
        assert_eq!(result.a_reg, 0b00); // 3 + 1 = 4, マスクされて 0
        assert_eq!(result.c_flag, true); // キャリーあり
        assert_eq!(result.pc_counter, 2); // PC + 1
    }
    ...
    fn test_execute_in() {
        ...
        let result = execute_in(&cpu, 0b10);
        assert_eq!(result.a_reg, 0b10); // 入力ポート値
        assert_eq!(result.c_flag, false); // キャリーリセット
        assert_eq!(result.pc_counter, 3); // PC + 1
    }
    ...
    fn test_execute_out() {
        ...
        let (result_cpu, output_value) = execute_out(&cpu, 0b11);
        assert_eq!(result_cpu.a_reg, 0b01); // A レジスタは変わらない
        assert_eq!(result_cpu.c_flag, false); // キャリーリセット
        assert_eq!(result_cpu.pc_counter, 0); // PC + 1 = 4, マスクされて 0
        assert_eq!(output_value, 0b11); // 出力値
    }
    ...
    fn test_execute_jnc_jump() {
        ...
        let result = execute_jnc(&cpu, 0b11);
        assert_eq!(result.pc_counter, 0b11); // アドレスにジャンプ
        assert_eq!(result.c_flag, false); // キャリーリセット
        assert_eq!(result.a_reg, 0b10); // A レジスタは変わらない
    }
    ...
    fn test_tick_add() {
        let mut machine = Machine::default();
        machine.rom[0] = 0b00_01; // ADD 1
        machine.cpu.a_reg = 0b10;

        let new_machine = tick(&machine);
        assert_eq!(new_machine.cpu.a_reg, 0b11); // 2 + 1 = 3
        assert_eq!(new_machine.cpu.c_flag, false);
        assert_eq!(new_machine.cpu.pc_counter, 1);
    }
    ...
    fn test_tick_in() {
        let mut machine = Machine::default();
        machine.rom[0] = 0b01_00; // IN
        machine.io_port.input_port = 0b10;

        let new_machine = tick(&machine);
        assert_eq!(new_machine.cpu.a_reg, 0b10); // 入力ポート値
        assert_eq!(new_machine.cpu.c_flag, false);
        assert_eq!(new_machine.cpu.pc_counter, 1);
    }
    ...
    fn test_tick_out() {
        let mut machine = Machine::default();
        machine.rom[0] = 0b10_11; // OUT 3

        let new_machine = tick(&machine);
        assert_eq!(new_machine.io_port.output_port, 0b11); // 出力ポート値
        assert_eq!(new_machine.cpu.c_flag, false);
        assert_eq!(new_machine.cpu.pc_counter, 1);
    }
    ...
    fn test_tick_jnc() {
        let mut machine = Machine::default();
        machine.rom[0] = 0b11_10; // JNC 2
        machine.cpu.c_flag = false; // ジャンプする

        let new_machine = tick(&machine);
        assert_eq!(new_machine.cpu.pc_counter, 0b10); // ジャンプ先アドレス
        assert_eq!(new_machine.cpu.c_flag, false);
    }
}
